/*! JRoll v2.3.0 ~ (c) 2015-2016 Author:BarZu Git:https://github.com/chjtx/JRoll Website:http://www.chjtx.com/JRoll/ */
/* global define*/
(function(window, document, Math) {
  ...
  utils = {
    ...
    moveTo: function(el, x, y, duration, callback) {
      ...
      result = /translate\(([\-\d\.]+)px,\s+([\-\d\.]+)px\)\s+translateZ\(0px\)\s+scale\(([\d\.]+)\)/g.exec(el.style[utils.TSF]);
      if (result) {
      ...
      function moving() {
        ...
        el.style[utils.TSF] = "translate(" + endX + "px, " + endY + "px) translateZ(0px) scale(" + zoom + ")";
  ...
  //添加监听事件
  document.addEventListener(utils.isMobile ? "touchstart" : "mousedown", _touchstart, false);
  document.addEventListener(utils.isMobile ? "touchmove" : "mousemove", _touchmove, false);
  document.addEventListener(utils.isMobile ? "touchend" : "mouseup", _touchend, false);
  if (utils.isMobile) {
    document.addEventListener("touchcancel", _touchend, false);
  } else {
    document.addEventListener(/firefox/.test(ua) ? "DOMMouseScroll" : "mousewheel", _wheel, false);
  }
  ...
  JRoll.prototype = {
    ...
    _scrollTo: function(x, y) {
      this.scroller.style[utils.TSF] = "translate(" + x + "px, " + y + "px) translateZ(0px) scale(" + this._z.scale + ")";
    },
    ...
    _move: function(e) {
      ...
      if (me.s === "scrollY") {
        ...
        me._doScroll(py, e);
          console.log('scrollY', y, me._s.startY, me._s.endY, me.y);
        return;
      }
...
console.log('me.y', me.y, me.x, me.maxScrollY);
